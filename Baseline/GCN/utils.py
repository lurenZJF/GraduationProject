...
def load_graph(path=None, dataset="HGAT"):
    ...
    return torch.FloatTensor(adj1), Labels, idx_train, idx_val, idx_test, idx_map_list[0]
...
def load_data():
    ...
    # 加载特征数据
    data = np.load("../HAN/feature.npz")
    features = data["feature"]
    # 加载图数据等
    graph, label, idx_train, idx_val, idx_test, idx_map_list = load_graph("../Dataset/Pre_data/")
    # 转化为tensor形式
    features = torch.FloatTensor(features)
    # mask操作
    train_mask = get_binary_mask(20531, idx_train)
    test_mask = get_binary_mask(20531, idx_test)
    val_mask = get_binary_mask(20531, idx_val)
