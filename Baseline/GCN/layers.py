...
class GraphConvolution(Module): 	# Module类的单继承
    ...
    def __init__(self, in_features, out_features, bias=True):
        super(GraphConvolution, self).__init__()  # super()函数是用于调用父类(超类)的方法
        # super().__init__()表示子类既能重写__init__()方法又能调用父类的方法
        self.in_features = in_features
        self.out_features = out_features
        # Parameter用于将参数自动加入到参数列表
        self.weight = Parameter(torch.FloatTensor(in_features, out_features))  # 先转化为张量，再转化为可训练的Parameter对象
        if bias:
    ...
    def forward(self, input, adj):  # 前向传播函数
        support = torch.mm(input, self.weight)  # input和self.weight矩阵相乘
        output = torch.spmm(adj, support)  # spmm()是稀疏矩阵乘法，减小运算复杂度
        if self.bias is not None:  # 变量是否不是None
